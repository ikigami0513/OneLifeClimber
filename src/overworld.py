    ...
    def create_path_sprites(self) -> None:
        ...
        for path_id, data in self.paths.items():
            ...
            for index, points in enumerate(path):
                if index < len(path) - 1:
                    start, end = pygame.Vector2(points), pygame.Vector2(path[index + 1])
                    path_dir = (end - start) / TILE_SIZE
                    start_tile = pygame.Vector2(int(start[0]/ TILE_SIZE), int(start[1]/ TILE_SIZE))

                    if path_dir.y:
                        dir_y = 1 if path_dir.y > 0 else -1
                        for y in range(dir_y, int(path_dir.y) + dir_y, dir_y):
                            path_tiles[path_id].append(start_tile + pygame.Vector2(0,y))

                    if path_dir.x:
                        dir_x = 1 if path_dir.x > 0 else -1
                        for x in range(dir_x, int(path_dir.x) + dir_x, dir_x):
                            path_tiles[path_id].append(start_tile + pygame.Vector2(x,0))
    ...
    def input(self) -> None:
        ...
        if self.current_node and not self.icon.path:
            if keys[pygame.K_s] and self.current_node.can_move('down'):
                self.move('down')
            if keys[pygame.K_q] and self.current_node.can_move('left'):
                self.move('left')
            if keys[pygame.K_d] and self.current_node.can_move('right'):
                self.move('right')
            if keys[pygame.K_z] and self.current_node.can_move('up'):
                self.move('up')
